...
print(f"unique words: {len(texts)}", flush=True)

jobs = []
for slow, folder in [(False, "google"), (True, "google_slow")]:
    out_dir = rf"{PROJECT}\assets\lector\{folder}"
    os.makedirs(out_dir, exist_ok=True)
    for key, ru in texts.items():
        out = rf"{out_dir}\{key}.mp3"
        if not os.path.exists(out):
            jobs.append((ru, slow, out))
print(f"jobs to do: {len(jobs)}", flush=True)

done = 0
failed = []
for ru, slow, out in jobs:
    ok = False
    for attempt in range(4):
        try:
            with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as tmp:
                tmp_path = tmp.name
            gTTS(text=ru, lang="ru", slow=slow).save(tmp_path)
            r = subprocess.run([
            ...
            if r.returncode == 0:
                ok = True
                break
            print(f"ffmpeg error for {ru}: {r.stderr.decode('utf-8', 'replace')[:150]}", flush=True)
        except Exception as e:
            wait = 5 * (attempt + 1)
            print(f"retry {attempt + 1} for {ru} ({e}), waiting {wait}s", flush=True)
            time.sleep(wait)
    if not ok:
        failed.append(ru)
    done += 1
